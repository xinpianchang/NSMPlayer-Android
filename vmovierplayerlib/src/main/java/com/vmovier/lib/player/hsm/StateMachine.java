...
//                            + "The start method not called, received msg: " + msg);
                    deferMessage(msg);
                    if (mDbg && mSm != null) mSm.logw("deferMessage under constructing: msg.what=" + msg.what);
                    return; // could do nothing anymore but return
                }
                performTransitions(msgProcessedState, msg);

                // FEATURE - author: tangye - added some final process
                if (mSm != null) {
                    mSm.performFinalMessage(msg);
                }
        ...
        private final void completeConstruction() {
            ...
            sendMessage(obtainMessage(SM_INIT_CMD, mSmHandlerObj));
        ...
        /** @see StateMachine#removeDeferredMessage(int) */
        private final void removeDeferredMessage(int what) {
            if (mDbg) mSm.log("removeDeferMessage: msg=" + what);

            Iterator<Message> i = mDeferredMessages.iterator();
            while (i.hasNext()) {
                Message m = i.next();
                if (m.what == what) {
                    i.remove();
    ...
    /**
     * @param what the message which need remove
     */
    protected final void removeDeferredMessage(int what) {
        mSmHandler.removeDeferredMessage(what);
    }
    ...
    /**
     * 当looper结束时, 执行特殊操作
     * @param msg 当前的Message
     */
    protected void performFinalMessage(Message msg) {}
}
